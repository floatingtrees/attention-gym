"""Benchmark forward and backward wall time of the SM100 CuTe SWA backend.
...
if __package__:
    from .benchmark_sliding_window_attention_cute import (
        make_inputs,
        useful_flops,
    )
else:
...
DIFFERENTIABLE_INPUTS = tuple(range(4))
...
def main() -> None:
    ...
    def forward() -> torch.Tensor:
        return sliding_window_attention(*inputs, backend="cute")
    ...
    print(f"window={args.window} rope_dims={args.rope_dims} share_kv={args.share_kv}")
