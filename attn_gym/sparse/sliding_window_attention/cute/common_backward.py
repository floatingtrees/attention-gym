...
class LocalNormBackward:
    ...
    def kernel(self, kv, weight, cos, sin, dy, dx, dweight):
        ...
        smem = cutlass.utils.SmemAllocator()
        sum_reduction = smem.allocate_tensor(
            Float32,
            cute.make_layout((1, (self.num_threads // cute.arch.WARP_SIZE, 1))),
            byte_alignment=16,
        )
        dot_reduction = smem.allocate_tensor(
            Float32,
            cute.make_layout((1, (self.num_threads // cute.arch.WARP_SIZE, 1))),
            byte_alignment=16,
        )
        sum_sq = row_reduce(
            local_sq, cute.ReductionOp.ADD, self.num_threads, sum_reduction, init_val=0.0
        )
        dot = row_reduce(
            local_dot, cute.ReductionOp.ADD, self.num_threads, dot_reduction, init_val=0.0
        )
